...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!rate_limit_check('reset_request', 3, 3600)) {
        $msg = 'تجاوزت عدد طلبات استعادة كلمة المرور. انتظر ساعة ثم حاول مرة أخرى.';
        $msgType = 'danger';
    } else {
        csrf_check();
...
<script>
    ...
    (function() {
        ...
        const tick = () => {
            ...
            if (diff < 5 * 60 * 1000) cd.classList.add('danger');
            else cd.classList.remove('danger');
        };
