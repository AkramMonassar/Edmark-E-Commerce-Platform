...
if (isset($_POST['addtocart'])) {
    ...
    if ($res && ($row = mysqli_fetch_assoc($res))) {
        $check = mysqli_query($con_db, "SELECT * FROM cart WHERE id = $pid");
        if ($check && mysqli_num_rows($check) == 0) {
            ...
            mysqli_query($con_db, "INSERT INTO cart (id, c_name, c_price, c_total, c_img) VALUES ($pid, '$name_esc', $price, $price, '$img_esc')");
        }
    }
}
